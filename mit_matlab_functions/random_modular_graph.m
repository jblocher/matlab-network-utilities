...
% Gergana Bounova, Last updated: October 19, 2009

function adj = random_modular_graph(n,c,p,r)
...
% assign modules: 1 -> n/c, n/c+1 -> 2n/c, ... , (c-1)n/c -> c(n/c);
modules=cell(c,1);
for k=1:c
...
for i=1:n
    for j=i+1:n
        ...
        if module_i==module_j
            if rand<=r*z/(n/c-1)      % prob of attachment within module
                adj(i,j)=1; adj(j,i)=1;
            end
        else
            if rand<=z*(1-r)/(n-n/c)  % prob of attachment across modules
                adj(i,j)=1; adj(j,i)=1;
            end
        end
    end
